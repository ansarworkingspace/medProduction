...
const ViewBlog = () => {
  ...
  const handleBookmarkClick = async () => {
    try {
      await axios.post(`${baseURL}/api/users/saveBlog/${selectedBlog._id}`, null, {
  ...
  useEffect(() => {
    axios.get(`${baseURL}/api/users/countLike/${blogId}`)
      .then(response => {
    ...
    axios.get(`${baseURL}/api/users/checkLike/${blogId}`, {
      withCredentials: true,
    })
      .then(response => {
  ...
  useEffect(() => {
    ...
    fetch(`${baseURL}/api/users/getOneBlog/${blogId}`) // Adjust the API route accordingly
  ...
  const handleLikeClick = async () => {
    try {
      await axios.post(`${baseURL}/api/users/likeBlog/${selectedBlog._id}`, null, {
        withCredentials: true,
      });
      ...
      {secondImageOrVideoExists && (
        <div className='imageView'>
          {getFileExtension(selectedBlog.images[1]) === 'mp4' ? (
            <video
              src={`${baseURL}/api/users/${selectedBlog.images[1]}`}
              controls
              className='viewImageOndiv'
            >
              Your browser does not support the video tag.
