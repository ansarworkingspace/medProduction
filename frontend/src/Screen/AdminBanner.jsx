...
const [banner, setBanner] = useState(null);
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      try {
        const response = await axios.get(`${baseURL}/api/admin/banners`);
    ...
    const fetchStatusSelected = async () => {
      try {
        const activeTrueBannerIdResponse = await axios.get(`${baseURL}/api/admin/checkActiveTrue`);
  ...
  const handleBannerUpload = async () => {
    ...
    try {
      ...
      await axios.post(`${baseURL}/api/admin/uploadBanner`, formData, {
        headers: {
  ...
  const handleDeleteBanner = async (bannerId) => {
    try {
      ...
      if (result.isConfirmed) {
        await axios.delete(`${baseURL}/api/admin/deleteBanner/${bannerId}`, {
          // headers: {
          //   Authorization: `Bearer ${token}`,
          // },
          withCredentials: true
...
const handleSelectBanner = async (bannerId) => {
  try {
    ...
    const response = await axios.post(
      `${baseURL}/api/admin/selectBanner/${bannerId}`,
      null,
      {
        headers: {
          Authorization: `Bearer ${token}`,
            ...
            <div className='bannerBox'>
              <div className='videoONbox'>
                <video style={{ borderRadius: "1rem" }}
                  src={`${baseURL}/api/users/${banner.media}`}
                  controls
