...
const AdminUserPRview = () => {
  ...
  useEffect(() => {
    const adminCheckAuth = async () => {
        try {
            const response = await fetch(`${baseURL}/api/admin/adminCheckAuth`, {
...
//passing admin jwt
  useEffect(() => {
    ...
    axios.get(`${baseURL}/api/admin/userProfile?email=${userEmail}`, {
    ...
    .then((response) => {
      ...
      axios.get(`${baseURL}/api/admin/allBlogs?email=${userEmail}`, {
        withCredentials: true, // Send credentials with the request
      })
      .then((response) => {
