...
const OtherUserPro = () => {
  ...
  const handleFollowClick = () => {
    if (isFollowing) {
      // Unfollow logic
      axios
        .post(`${baseURL}/api/users/unfollow/${userId}`, null, {
      ...
      // Follow logic
      axios
        .post(`${baseURL}/api/users/follow/${userId}`, null, {
          withCredentials: true,
        })
        .then(() => {
          setIsFollowing(true);
  ...
  useEffect(() => {
    ...
    axios.get(`${baseURL}/api/users/checkFollowing/${userId}`, {
      withCredentials: true,
    })
    .then(response => {
  ...
  useEffect(() => {
    axios
      .get(`${baseURL}/api/users/authorProfile/${userId}`)
...
// Fetch user's blogs
axios
.get(`${baseURL}/api/users/getUserBlogs/${userId}`,{
withCredentials:true
})
.then(response => {
...
.catch(error => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${baseURL}/api/users/followerFollowingCount/${userId}`, {
        withCredentials: true,
      })
      .then(response => {
  ...
  const handleChatButtonClick = async () => {
    try {
      const response = await axios.get(
        `${baseURL}/api/users/createOrGetChatRoom/${userId}`,
        { withCredentials: true }
      );
    ...
    <div className="profile-container">
      ...
      <div className="profile-picture">
        ...
        {userDetails.profileImage ? (
                <Image
                src={`${baseURL}/api/users/uploads/${userDetails.profileImage}`}
                  alt="Profile"
                ...
                <div className='postImage'  style={{backgroundColor:"#181a1b"}} >
                  ...
                  {blog.images.length > 0 && (
                    ...
                    getFileExtension(blog.images[0]) === 'mp4' ? (
                      <video
                        src={`${baseURL}/api/users/${blog.images[0]}`}
                        controls
