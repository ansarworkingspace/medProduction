...
const LandingPage = () => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          const blogsResponse = await axios.get(`${baseURL}/api/users/allBlogsLanding`);
          setBlogs(blogsResponse.data);
    
          try {
            const selectedBannerResponse = await axios.get(`${baseURL}/api/users/selectedBanner`);
...
<div className="videoOverlay"></div> {/* Add this overlay */}
    <video
      src={`${baseURL}/api/users/${selectedBannerMedia}`}
      autoPlay
      loop
      muted
