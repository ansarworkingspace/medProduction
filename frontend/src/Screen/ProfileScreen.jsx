...
useEffect(() => {
  const checkAuth = async () => {
      try {
          const response = await fetch(`${baseURL}/api/users/checkAuth`, {
...
//user following count
  useEffect(() => {
    ...
    axios
      .get(`${baseURL}/api/users/followerFollowingCount/${userInfo._id}`, {
...
//check user status
  useEffect(() => {
    const fetchUserStatus = async () => {
      try {
        const response = await fetch(`${baseURL}/api/users/status/${userInfo._id}`);
  ...
  const handleDeleteBlog = async (blogId) => {
    try {
      ...
      if (result.isConfirmed) {
        await axios.delete(`${baseURL}/api/users/deleteBlog/${blogId}`, {
          withCredentials: true,
        });
...
? `${baseURL}/api/users/uploads/${userInfo.profileImage}`
...
const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        const response = await axios.get(`${baseURL}/api/users/blogs`, {
