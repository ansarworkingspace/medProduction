...
async function makeNotification(messageId, newMessageRoomId) {
  try {
    ...
    const response = await axios.post(`${baseURL}/api/users/updateNoti/${newMessageRoomId}`, { messageId });
  ...
  const fetchMessages = async () => {
    try {
      const response = await axios.get(
        `${baseURL}/api/users/chatMessages/${chatRoomId}`,
        {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const { data } = await axios.post(
        `${baseURL}/api/users/chatSend/${chatRoomId}`,
        {
          content: message,
        },
        {
...
useEffect(() => {
  if (chatRoomId) {
    axios.get(`${baseURL}/api/users/participants/${chatRoomId}`, {
      withCredentials: true
    })
      .then(response => {
...
const typingHandler = (e) => {
  ...
  return (
    <div className='chatPageContainer'  style={{ maxHeight: "400px", position: "relative" }}>
      ...
      <Image style={{width:"43%",height:"80%"}} src={`${baseURL}/api/users/uploads/${selectedChat.profileImage}`} alt="Profile" className="followingImage" roundedCircle />
    ) : (
