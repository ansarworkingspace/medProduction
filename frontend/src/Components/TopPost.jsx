...
const TopPost = () => {
  ...
  useEffect(() => {
    const fetchTopBlogs = async () => {
      try {
        const response = await axios.get(`${baseURL}/api/users/topThreepost`);
  ...
  return (
   ...
   {topBlogs.map((blog) => (
        <div className='topPostCard' key={blog._id} style={{ backgroundImage: `url(${baseURL}/api/users/${blog.images[0]})` }}>
